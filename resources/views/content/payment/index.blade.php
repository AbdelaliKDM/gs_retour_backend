  ...
  <script>
    ...
    $(document.body).on('click', '.info', function () {
      ...
      success: function (response) {
        ...
        var modal = data.type == 'wallet' ? $('#wallet-info-modal') : $('#invoice-info-modal');
        ...
        modal.find('.payment-method')
          .removeClass('bg-primary bg-secondary bg-success bg-warning bg-danger bg-info bg-dark')
          .addClass(statusClass)
        ...
        if (data.receipt) {
          ...
          modal.find('.payment-receipt')
          .attr('href', data.receipt)
          .text("{{__('app.view')}}")
        ...
        if (data.type == 'wallet') {
          // Wallet information
          modal.find('.wallet-balance').text(`${data.payable.balance_money}`);
          modal.find('.wallet-charges').text(data.payable.charges);
        } else if (data.type == 'invoice') {
          // Invoice information
          modal.find('.invoice-total-amount').text(`${data.payable.total_amount_money}`);
          modal.find('.invoice-tax-amount').text(`${data.payable.tax_amount_money}`);
          modal.find('.invoice-month').text(data.payable.month + ' ' + data.payable.year);
        }
